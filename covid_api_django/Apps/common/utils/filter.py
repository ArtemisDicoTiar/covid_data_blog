...
def lstm_prediction_parameter_validate(predictedDate: str, date: str, offset: int):
    if offset > 28:
        return HttpResponseBadRequest("Offset should be smaller than 28 days")
    ...
    if (datetime.strptime(date, '%Y-%m-%d') + timedelta(days=offset - 1)).date() \
            - datetime.strptime(predictedDate, '%Y-%m-%d').date() \
            > timedelta(days=28):
        return HttpResponseBadRequest("Only 28 days can be viewed from predicted date")
